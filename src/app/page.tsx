...
import { FileText, Send, Upload, X, Sparkles, ChevronDown, Trash2, AlertCircle, Menu, ArrowLeft, Plus, Paperclip } from 'lucide-react';
                      ...
                      <div className="absolute inset-0 z-40 flex">
                                ...
                                <div className="relative z-50 w-[80vw] max-w-xs bg-[#17171f] h-full flex flex-col shadow-2xl">
                                            ...
                                            <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-2">
                                                {docs.length === 0 && <p className="text-center text-white/30 text-xs mt-8">No documents yet</p>p>}
                                                {docs.map((doc) => (
                                          <button key={doc.id} onClick={() => { setActiveDocId(doc.id); setMessages([]); setView('chat'); setShowSidebar(false); }}
                                                                className={cn('w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-left transition-all', doc.id === activeDocId ? 'bg-violet-600/20 border border-violet-500/30' : 'bg-white/5 border border-transparent')}>
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                                ...
                                                                                <p className="text-[11px] text-white/40">{formatBytes(doc.size)}{doc.pages ? ' · ' + doc.pages + 'p' : ''}</p>p>
                                                            </div>div>
                                            ...
                                            <div className="px-5 mb-6">
                                                          <div {...getRootProps()} className={cn('relative rounded-3xl border-2 border-dashed transition-all cursor-pointer overflow-hidden', isDragActive ? 'border-violet-500 bg-violet-500/10' : 'border-white/15 bg-white/4')}>
                                                                          ...
                                                                          <div className="px-6 py-8 flex flex-col items-center gap-4">
                                                                                            <div className={cn('w-16 h-16 rounded-2xl flex items-center justify-center transition-all', isDragActive ? 'bg-violet-500/30' : 'bg-white/8')}>
                                                                                                {isUploading ? (
                                                ...
                                                <Upload size={24} className={isDragActive ? 'text-violet-400' : 'text-white/50'} />
                                                                                            ...
                                                                                            <div className="text-center">
                                                                                                                <p className="text-white font-semibold text-base">{isUploading ? 'Uploading...' : isDragActive ? 'Drop to upload' : 'Tap to upload PDF'}</p>p>
                                                                                                                <p className="text-white/40 text-xs mt-1">Max 10 MB</p>p>
                                                                                                </div>div>
                                                                              {!isUploading && (
                                              <div className="px-6 py-2.5 rounded-full bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-sm font-semibold">Choose file</div>div>
                                                                                            )}
                                                                          </div>div>
                                                          </div>div>
                                                {uploadError && (
                                                            ...
                                                            {docs.slice(0, 3).map((doc) => (
                                                                <button key={doc.id} onClick={() => { setActiveDocId(doc.id); setMessages([]); setView('chat'); }}
                                                                                      ...
                                                                                      <div className="flex-1 min-w-0 text-left">
                                                                                                              <p className="text-sm font-medium text-white truncate">{doc.name}</p>p>
                                                                                                              <p className="text-[11px] text-white/40">{formatBytes(doc.size)}{doc.pages ? ' · ' + doc.pages + ' pages' : ''}</p>p>
                                                                                          </div>div>
                                ...
                                <div className="relative px-4 pt-12 pb-3 flex items-center gap-3 border-b border-white/8 bg-[#0f0f13]/90 backdrop-blur-md">
                                    ...
                                    {activeDoc && (
                                        <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-[11px] text-white/40">{formatBytes(activeDoc.size)}{activeDoc.pages ? ' · ' + activeDoc.pages + 'p' : ''}</p>p>
                                        </div>div>
                                            )}
                                            <button onClick={() => setShowSidebar(true)} className="w-9 h-9 rounded-full bg-white/8 flex items-center justify-center">
                                                          <Menu size={16} className="text-white/70" />
                                    ...
                                    {messages.map((msg) => (
                                        <div key={msg.id} className={cn('flex gap-2', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                                            {msg.role === 'assistant' && (
                                                        ...
                                                        <div className={cn('max-w-[82%] rounded-3xl px-4 py-3',
                                                                                             msg.role === 'user' ? 'bg-gradient-to-br from-violet-600 to-indigo-600 text-white rounded-tr-sm' : 'bg-white/8 border border-white/10 text-white/90 rounded-tl-sm')}>
                                                            {msg.role === 'user' ? (
                                                                <p className="text-sm leading-relaxed">{msg.content}</p>p>
                                                              ) : (
                                                                <div className="text-sm leading-relaxed prose prose-invert prose-sm max-w-none prose-p:my-1 prose-headings:text-white prose-a:text-violet-400 prose-code:text-violet-300 prose-pre:bg-black/30 prose-pre:rounded-xl prose-blockquote:border-violet-500/50">
                                        ...
                                        <div className="flex items-end gap-2">
                                                        ...
                                                        <button onClick={() => handleSend()} disabled={!input.trim() || isLoading}
                                                                              className={cn('w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 transition-all',
                                                                                                                input.trim() && !isLoading ? 'bg-gradient-to-br from-violet-600 to-indigo-600 shadow-lg shadow-violet-500/30' : 'bg-white/10')}>
